...
class IconItemDelegate(QtWidgets.QStyledItemDelegate):
    ...
    def paint(self, painter, option, index):
        ...
        _data = index.data()
        _id = _data["Id"]
        _asset = zfused_api.asset.Asset(_id)
        # _project_entity_handle = zfused_api.objects.Objects(_asset.data()["ProjectEntityType"], _asset.data()["ProjectEntityId"])
        # _project_step_handle = zfused_api.step.ProjectStep(_asset.data()["ProjectStepId"])
        _name = _asset.full_name_code().replace("/","_")
        ...
        _link_full_name = _asset.full_name().replace("/"," - ") # _asset.name_code()
        ...
        _step_rect = QtCore.QRectF( _link_rect.x(),
                                   ...
                                   _link_rect.width(),
                                   20 )
        _project_step_name = _asset.code()
        ...
        #  绘制任务状态
        _status_handle = zfused_api.status.Status(_asset.data()["StatusId"])
