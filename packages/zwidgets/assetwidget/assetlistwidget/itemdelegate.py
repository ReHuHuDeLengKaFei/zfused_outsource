...
class ItemDelegate(QtWidgets.QStyledItemDelegate):
    ...
    def paint(self, painter, option, index):
        _asset_data = index.data()
        _asset_id = _asset_data["Id"]
        _asset_handle = zfused_api.asset.Asset(_asset_id, _asset_data)
        #_name = _asset_handle.full_name_code().replace("/","_")
        ...
        _pixmap = _pixmap = cache.ThumbnailCache.get_pixmap(_asset_handle, self.parent().update)
        if _pixmap:
            ...
            painter.setBrush(QtGui.QColor(color.LetterColor.color(_asset_handle.code().lower()[0])))
        ...
        _name = _asset_handle.full_name().replace("/"," - ")
        ...
        #                 u"{} 版本".format(len(_asset_handle.data())) )

        # # painter status
        # _fm = QtGui.QFontMetrics(_font)
        # _status_id = _asset_handle.status_id()
