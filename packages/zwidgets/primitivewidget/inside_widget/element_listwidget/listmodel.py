...
class ListFilterProxyModel(QtCore.QSortFilterProxyModel):
    ...
    def _search(self, data):
        if not self._search_text:
            return True
        _id = data["task_id"]
        _task = zfused_api.task.Task(_id)
        _in_name = self._search_text.lower() in _task.name_code().lower() or self._search_text.lower() in _task.project_entity().name_code().lower()
        if _in_name:
            return True
        return False
    ...
    def _filter_type(self, data):
        if not self._filter_type_list:
            return True
        if zfused_api.task.Task(data.get("Id")).project_entity().type_id() in self._filter_type_list:
            return True
        return False
    ...
    def _filter_project_step(self, data):
        ...
        if not self._filter_project_step_list:
            return True
        _project_step_id = data["project_step_id"]
        if data["project_step_id"] in self._filter_project_step_list:
            return True
        return False
        ...
        # _task = zfused_api.task.Task(data.get("Id"))
