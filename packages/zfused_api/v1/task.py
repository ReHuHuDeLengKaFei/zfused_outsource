...
def new_task(name, link_obj, link_id, project_step_id, status_id, assigned_id, start_time, due_time, description=None,
    ...
    _task, _status = zfused_api.zFused.post(key="task", data={
        "SelfObject": "task",
        "Name": name,
...
class Task(_Entity):
    ...
    def time_nodes(self):
        if not self._time_nodes_calculate:
            ...
            self._time_nodes = self.get("time_node", filter={
                "Object": "task",
                "ObjectId": self._id})
    ...
    def file_code(self):
        ...
        _project_entity = zfused_api.zFused.get("project_entity", filter={
            "ProjectId": self._data.get("ProjectId"),
            "Code": self.object()})
        if _project_entity:
    ...
    def notes(self):
        ...
        """
        _notes = []
        _historys = self.get("task_history", filter={
            "TaskId": self._id})
        if not _historys:
            _data = self._data
            _note = {
                "user_id": _data["CreatedBy"],
                "notes": _data,
                "type": "create",
    ...
    def submit_external_approval(self, name,
        ...
        _task_id = self._id
        # index
        _all_version = self.get("version", filter={
        ...
        if _all_version:
            ...
            _index = 1
        if _index:
            _last_version, _status = self.post(key="version", data={
                "Name": name,
                "ProjectId": _project_id,
                "StepId": _step_id,
                "ProjectStepId": _project_step_id,
        ...
        # 提交审查人员
        _value, _status = self.post(key="approval", data={
            "Object": "version",
        ...
        _group_users = self.get("group_user", filter={
            "EntityType": "task",
            "EntityId": self._id})
        if _group_users:
        ...
        if _approval_status_ids:
            ...
            _status_list = zfused_api.zFused.get("project_status", filter={
                "ProjectId": self._data["ProjectId"],
                "StatusId__in": "|".join(_strs)})
            if _status_list:
    ...
    def submit_review(self, name,
        ...
        # index
        _all_review = self.get("report", filter={
            "TaskId": _task_id})
        _index = 0
        if _all_review:
        ...
        _layer_index = 0
        if _layer_id:
            _layers = self.get("report", filter={
                "TaskId": _task_id,
                "LayerId": _layer_id})
            if _layers:
        ...
        # 提交审查人员
        _text, _status = self.post(key="review", data={
            "EntityType": "report",
            "EntityId": _report_id,
        ...
        # 提交审核记录
        _, _status = self.post(key="review_record", data={
            "ReviewId": _review_id,
            "ReviewProcessId": review_process_id,
            "CreatedBy": _user_id,
            "CreatedTime": _create_time})
    ...
    def analy_is_production(self):
        ...
        # get input versions
        _versions = self.get("relative", filter={
        ...
        _is_production = 1
        for _version in _versions:
            _version = self.get("version", filter={
                "Id": _version["SourceId"]})[0]
            _task = self.get("task", filter={
                "Id": _version["TaskId"]})[0]
            _versions = self.get("version", filter={
                "TaskId": _task["Id"]})
        ...
        if _is_new_attribute_solution:
            ...
            for _input_attr in _input_attrs:
                ...
                if _input_attr["Mode"] == "indirect":
                    _attr = self.get("step_attr_output", filter={
                        "Id": _input_attr["StepAttrId"]})
                    if not _attr:
                        continue
                    _attr_project_step_id = _attr[0]["ProjectStepId"]
                    task = self.get("task", filter={
                        "ProjectStepId": _attr_project_step_id,
                        "ProjectEntityId": self._data["ProjectEntityId"]})
                    # 任务状态机制?
                    _is_next = False
                    if task:
                    ...
                    if _is_next:
                        ...
                        # 获取关联任务的步骤
                        # 为了配合有的有解算有的无解算的问题？？？
                        _step_attr = self.get("step_attr_input", filter={
                            "ProjectStepId": _attr_project_step_id,
                            "Mode": "direct"})
                        if _step_attr:
                            _project_step_id = self.get("step_attr_output", filter={
                                "Id": _step_attr[0]["StepAttrId"]})[0]["ProjectStepId"]
                            _task = self.get("task", filter={
                                "ProjectEntityId": self._data["ProjectEntityId"],
                                "ProjectStepId": _project_step_id})
                            if _task:
                    ...
                    if not _attr:
                        continue
                    _attr_project_step_id = _attr[0]["ProjectStepId"]
                    projectStepHandle = self.get("project_step", filter={
                        "Id": _attr_project_step_id})[0]
                    tarStepHandle = self.get("project_step", filter={
                        "Id": self._data["ProjectStepId"]})[0]
                    # get relative object
                    relatives = self.get("relative", filter={
                        "SourceObject": projectStepHandle["Object"],
    ...
    def target_relatives(self):
        ...
        _relatives = zfused_api.zFused.get("relative", filter={
            "SourceObject": "task",
            "SourceId": self._id})
        return list(set([(_relative["TargetObject"], _relative["TargetId"]) for _relative in _relatives if
                         _relative["TargetId"] != self._id] if _relatives else []))
    ...
    def history(self):
        ...
        :rtype: list
        """
        _history = self.get("task_history", filter={
            "TaskId": self._id}, sortby=["ChangeTime"], order=["asc"])
        if _history:
            return _history
        else:
            return []
    ...
    def analy_working_time(self):
        ...
        if _time_key:
            for i in range(int(len(_time_key) / 2)):
                ...
                _hours_handle_1 = worktime.WorkHours(
                    datetime.datetime(int(_t_s[0]), int(_t_s[1]), int(_t_s[2]), int(_h_s[0]), int(_h_s[1])),
                    datetime.datetime(int(_t_e[0]), int(_t_e[1]), int(_t_e[2]), int(_h_e[0]), int(_h_e[1])),
                    worktiming=["09:30", "12:00"])
                _hours_handle_2 = worktime.WorkHours(
                    datetime.datetime(int(_t_s[0]), int(_t_s[1]), int(_t_s[2]), int(_h_s[0]), int(_h_s[1])),
                    datetime.datetime(int(_t_e[0]), int(_t_e[1]), int(_t_e[2]), int(_h_e[0]), int(_h_e[1])),
    ...
    def submit_task_time(self, date, hours, description=""):
        ...
        _times = zfused_api.zFused.get("task_time", filter={
            "UserId": _user_id,
        ...
        if _times:
            ...
            _value, _status = zfused_api.zFused.post("task_time", data={
                "UserId": _user_id,
    ...
    def prophet(self, proposed_entity_type, proposed_entity_id, proposed_description):
        ...
        _user_id = zfused_api.zFused.USER_ID
        # new prophet
        _value, _status = zfused_api.zFused.post("prophet", data={
            "EntityType": "task",
            "EntityId": self._id,
            "Value": -1,
