...
def get_cloud_trans_server_addr():
    import zfused_api
...
def send_file_to_cloud(src_file, dst_file, progress = [0]):
    if is_hash_equal(src_file, dst_file):
        return True

    _socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    _socket.connect(get_cloud_trans_server_addr())
    try:
        _src_file = src_file
        _dst_file = dst_file
        _file_size = os.path.getsize(_src_file)
...
def get_file_from_cloud(src_file, dst_file, progress = [0]):
    if is_hash_equal(src_file, dst_file):
        return True
