...
def send_message(submitter_object, submitter_id, receiver_id, data, msgtype = "", link_object = "", link_id = 0, group_type = "", group_id = 0):
    ...
    if _status:
        _id = _value["Id"]
        if receiver_id:
            for _receiver_id in receiver_id:
                _is_read = "0"
...
# class MessageLink(zfused_api.zFused):
#     global_dict = {}
...
#         self._data = data
    ...
    def is_read(self):
        return int(self._data["IsRead"])

    def update_is_read(self, is_read = 1):
        logger.info("update is read")
