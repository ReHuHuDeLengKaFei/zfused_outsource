...
class Playblast(window._Window):
    ...
    def _playblast(self, path):
        ...
        for i in range(_min_time, _max_time + 1):
            ...
            self.process_image(_progress_image, i)
        ...
        if sound:
            ...
            if os.path.isfile(sound_path):
                self.conver_image_mov(production_image_path, _video_path, self._temp_config.get("fps"), _time,
                                      sound=sound_path, start_frame=_min_time)
        else:
            self.conver_image_mov(production_image_path, _video_path, self._temp_config.get("fps"), _time,
                                  start_frame=_min_time)
        # 删除临时路径及文件
        try:
    ...
    def conver_image_mov(self, image_path, outpath, fps, time, sound="", start_frame=101):
        ...
        _command = '{} -framerate {} -start_number {} -i {} -r {} -t {} -vcodec  h264 -acodec copy -b:v 18000k -y {}'.format(_FFMPEG,
                                                                                                                fps,
                                                                                                                start_frame,
                                                                                                                image_path,
                                                                                                                fps,
        ...
        if os.path.exists(sound):
            _command = '{} -framerate {} -start_number {} -i {} -i {} -r {} -t {} -vcodec  h264 -acodec copy -b:v 18000k -y {}'.format(
                _FFMPEG, fps, start_frame, image_path, sound, fps, time, outpath)
            # command = '{} -framerate {} -start_number {} -i {} -i {} -r {} -t {} -vcodec libx264 -crf 12 -pix_fmt yuv420p -y {}'.format(
            #     _FFMPEG, fps, start_frame, image_path, sound, fps, time, outpath)
    ...
    def process_image(self, image_path, current_frame):
    ...
    def painter(self, painter, rect=None):
        ...
        _font.setPixelSize(self._temp_config.get("font-size") * self._scale)
        painter.setFont(_font)
        _fm = QtGui.QFontMetrics(_font)
        
        painter.setPen(QtGui.QPen(QtGui.QColor("#BEBEBE")))
        painter.drawRect(_width * 0.05, _height * 0.05, _width * 0.9, _height * 0.9)
        
        _margin = self._temp_config.get("margin")
        _text_height = self._temp_config.get("text-height") * self._scale
    ...
    #     self.resize( self.playblast_widget.width(),
    #                  self.playblast_widget.height() )
    ...
    def _resize(self):
        ...
        self.playblast_widget.resize(new_size.width(), new_size.height() + self.playblast_widget.operation_widget.height())
        self.resize(self.playblast_widget.width(), self.playblast_widget.height())
    ...
    def _resolution_scale(self, _scale):
        ...
        self._temp_config["image_size"] = [int(self._temp_config.get("image_size")[0] * self._scale),
                                           int(self._temp_config.get("image_size")[1] * self._scale)]
        ...
        for _index, _hud in enumerate(_huds):
            if _hud.get("code") == "image_size":
                self._temp_config["hud"][_index]["cmd"] = "text = '{} x {}'".format(
                    int(self._temp_config["image_size"][0]), int(self._temp_config["image_size"][1]))
                break
    ...
    def _build(self):
        ...
        self.set_help_url("tool/utility/playblast")
