...
class Ui(object):
    def showUi(self):
        ...
        self.creatTRCL = cmds.rowColumnLayout(parent=self.creatFrL, numberOfColumns=5,
                                              columnSpacing=[(1, 8), (2, 7), (3, 7), (4, 6)])
        ...
        self.add_Frl = cmds.frameLayout(parent=self.windowFoL,label =u'新增设置')
        self.add_Trcl = cmds.columnLayout(parent = self.add_Frl,columnAttach=('both', 5), rowSpacing=0, columnWidth=360)
        # self.add_Trcl = cmds.columnLayout(parent = self.add_Frl)




        self.proxyFrL = cmds.frameLayout(parent=self.add_Trcl, label='add to proxy group')
        self.proxyTRCL = cmds.rowColumnLayout(parent=self.proxyFrL, numberOfColumns=1)
        self.proxyBut = cmds.button(parent=self.add_Trcl, label=u'proxy添加', h=30, width=330,
                                    command=lambda *args: self.snakeCommand(self.proxyBut))
        #cmds.formLayout(parent=self.proxyTRCL)

        self.fix_layout = cmds.frameLayout(parent=self.add_Trcl, label='fix group attr')
        self.fix_trcl = cmds.rowColumnLayout(parent=self.fix_layout, numberOfColumns=1)
        fix_but = cmds.button(parent=self.add_Trcl, label=u'修复组', h=30, width=330, command=lambda *args: self.fix_attr())
        self.setNameSpace()
        self.characterComp()
        self.GRPExists()
    ...
    def characterComp(self):
        ...
        cmds.formLayout(self.windowFoL, edit=True, attachForm=[
            ...
            (self.add_Frl,'bottom',5)
            # (self.controlFrL, 'bottom', 20),
            # (self.proxyFrL, 'left', 5),
            # (self.proxyFrL, 'right', 5),
            # (self.proxyFrL, 'top', 100),
            # (self.fix_layout, 'bottom', 5),
            # (self.fix_layout,'left',5),
            # (self.fix_layout,'right',5)
        ],
                        attachControl=[
                            (self.controlFrL, 'top', 5, self.creatFrL),
                            # (self.fix_layout,'top',55,self.creatFrL)
                        ])
        # cmds.formLayout(self.proxyFrL,edit =True,attachForm = [(self.fix_layout,'left',5),(self.fix_layout,'right',5),(self.fix_layout,'bottom',5)])
    ...
    def propComp(self):
        controlFrLP = cmds.frameLayout(parent=self.windowFoL, label='Control', collapsable=True, collapse=True)
        controlFoLP = cmds.formLayout(parent=controlFrLP)
        cmds.formLayout(self.windowFoL, edit=True, attachForm=[
            (self.creatFrL, 'top', 5),
            (self.creatFrL, 'left', 5),
            (self.creatFrL, 'right', 5),
            ...
            (controlFrLP, 'right', 5),
            (self.add_Frl, 'bottom', 5)
            # (controlFrLP, 'bottom', 20),
            # (self.proxyFrL, 'left', 5),
            # (self.proxyFrL, 'right', 5),
            # (self.proxyFrL, 'top', 100),
            # (self.proxyFrL, 'bottom', 5),
            # (self.fix_layout, 'bottom', 5),
            # (self.fix_layout, 'left', 5),
            # (self.fix_layout, 'right', 5)
        ],
                        attachControl=[
                            (controlFrLP, 'top', 5, self.creatFrL)
                        ])
        cmds.showWindow(self.creatTWindow)
        self.controlFrL = controlFrLP

        propTRCL = cmds.rowColumnLayout(parent=controlFoLP, numberOfRows=3, rowSpacing=[(1, 0), (2, 3)])
        porpAssBtn = cmds.button('propBtn', l='geometry', h=50, w=330, p=propTRCL,
                                 c=lambda *args: self.snakeCommand(porpAssBtn))
        previewBtn = cmds.button('proppreviewBtn', l='preview', h=50, w=330, p=propTRCL,
                                 c=lambda *args: self.snakeCommand(previewBtn))
    ...
    def Daniel_modleAss(self):
        ...
        hairAssBut = cmds.button('hairBut', l="hair", w=100, h=30, bgc=[0.9, 0.4, 0],
                                 command=lambda *args: self.snakeCommand(hairAssBut))
        beardAssBut = cmds.button('beardBut', l='beard', w=50, h=50, bgc=[0.9, 0.9, 0.9],
                                  command=lambda *args: self.snakeCommand(beardAssBut))
        headBut = cmds.button('head', l='head', w=80, h=30, bgc=[0.9, 0.9, 0.9],
                              command=lambda *args: self.snakeCommand(headBut))
        r_earAssBut = cmds.button('r_earBut', l='r_ear', w=50, h=60, bgc=[0.9, 0.4, 0],
                                  command=lambda *args: self.snakeCommand(r_earAssBut))
        l_earAssBut = cmds.button('l_earBut', l='l_ear', w=50, h=60, bgc=[0.9, 0.4, 0],
                                  command=lambda *args: self.snakeCommand(l_earAssBut))
        r_elbowAssBut = cmds.button('r_elbowBut', l='r_brow', w=60, h=20, bgc=[0.9, 0.9, 0.9],
                                    command=lambda *args: self.snakeCommand(r_elbowAssBut))
        l_elbowAssBut = cmds.button('l_elbowBut', l='l_brow', w=60, h=20, bgc=[0.9, 0.9, 0.9],
                                    command=lambda *args: self.snakeCommand(l_elbowAssBut))
        r_eyeAssBut = cmds.button('r_eyeBut', l='r_eye', w=60, h=20, bgc=[0.9, 0.9, 0.9],
                                  command=lambda *args: self.snakeCommand(r_eyeAssBut))
        l_eyeAssBut = cmds.button('l_eyeBut', l='l_eye', w=60, h=20, bgc=[0.9, 0.9, 0.9],
                                  command=lambda *args: self.snakeCommand(l_eyeAssBut))
        r_upLassAssBut = cmds.button('r_upLassBut', l='r_upLash', w=60, h=20, bgc=[0.9, 0.4, 0],
                                     command=lambda *args: self.lashCommand_R())
        l_upLassAssBut = cmds.button('l_upLassBut', l='l_upLash', w=60, h=20, bgc=[0.9, 0.4, 0],
                                     command=lambda *args: self.lashCommand_L())
        r_lowLassAssBut = cmds.button('r_lowLassBut', l='r_lowLash', w=60, h=20, bgc=[0.9, 0.4, 0],
                                      command=lambda *args: self.lashCommand_R())
        l_lowLassAssBut = cmds.button('l_lowLassBut', l='l_lowLash', w=60, h=20, bgc=[0.9, 0.4, 0],
                                      command=lambda *args: self.lashCommand_L())
        upTeeAssBut = cmds.button('up_teeBut', l='upteeth', w=100, h=20, bgc=[0.9, 0.9, 0.9],
                                  command=lambda *args: self.snakeCommand(upTeeAssBut))
        tongueAssBut = cmds.button('tongueBut', l='tongue', w=100, h=20, bgc=[0.9, 0.4, 0],
                                   command=lambda *args: self.snakeCommand(tongueAssBut))
        lowTeeAssBut = cmds.button('low_teeBut', l='lowteeth', w=100, h=20, bgc=[0.9, 0.9, 0.9],
                                   command=lambda *args: self.snakeCommand(lowTeeAssBut))
        nonnasalityBut = cmds.button('n_lity', l='nonnasality', w=100, h=20, bgc=[0.9, 0.4, 0.0],
                                     command=lambda *args: self.snakeCommand(nonnasalityBut))
        bodyBut = cmds.button('bodyBut', l='body', w=80, h=50, bgc=[0.9, 0.4, 0],
                              command=lambda *args: self.snakeCommand(bodyBut))
        bodyassBut = cmds.button('bodyassBut', l='bodyass', w=80, h=40, bgc=[0.9, 0.9, 0.9],
                                 command=lambda *args: self.snakeCommand(bodyassBut))
        closeeyeBut = cmds.button('closeeyeBut', l='closeeye', w=80, h=30, bgc=[0.9, 0.9, 0.9],
                                  command=lambda *args: self.snakeCommand(closeeyeBut))
        openeyeBut = cmds.button('openeyeBut', l='openeye', w=80, h=30, bgc=[0.9, 0.9, 0.9],
                                 command=lambda *args: self.snakeCommand(openeyeBut))

        r_armAssBut = cmds.button('r_armBut', l='r_arm', w=50, h=30, bgc=[0.9, 0.9, 0.9],
                                  command=lambda *args: self.snakeCommand(r_armAssBut))
        l_armAssBut = cmds.button('l_armBut', l='l_arm', w=50, h=30, bgc=[0.9, 0.9, 0.9],
                                  command=lambda *args: self.snakeCommand(l_armAssBut))
        r_handAssBut = cmds.button('r_handBut', l='r_hand', w=50, h=40, bgc=[0.9, 0.9, 0.9],
                                   command=lambda *args: self.snakeCommand(r_handAssBut))
        l_handAssBut = cmds.button('l_handBut', l='l_hand', w=50, h=40, bgc=[0.9, 0.9, 0.9],
                                   command=lambda *args: self.snakeCommand(l_handAssBut))
        r_legAssBut = cmds.button('r_legBut', l='r_leg', w=40, h=40, bgc=[0.9, 0.9, 0.9],
                                  command=lambda *args: self.snakeCommand(r_legAssBut))
        l_legAssBut = cmds.button('l_legBut', l='l_leg', w=40, h=40, bgc=[0.9, 0.9, 0.9],
                                  command=lambda *args: self.snakeCommand(l_legAssBut))
        r_footAssBut = cmds.button('r_footBut', l='r_foot', w=50, h=40, bgc=[0.9, 0.4, 0],
                                   command=lambda *args: self.snakeCommand(r_footAssBut))
        l_footAssBut = cmds.button('l_footBut', l='l_foot', w=50, h=40, bgc=[0.9, 0.4, 0],
                                   command=lambda *args: self.snakeCommand(l_footAssBut))
        r_shoeAssBut = cmds.button('r_shoeBut', l='r_shoe', w=50, h=20, bgc=[0.9, 0.4, 0],
                                   command=lambda *args: self.snakeCommand(r_shoeAssBut))
        l_shoeAssBut = cmds.button('l_shoeBut', l='l_shoe', w=50, h=20, bgc=[0.9, 0.4, 0],
                                   command=lambda *args: self.snakeCommand(l_shoeAssBut))

        headassBut = cmds.button('headBut', l='headass', parent=f, w=100, h=30, bgc=[0.9, 0.9, 0.9],
                                 command=lambda *args: self.snakeCommand(headassBut))
        clothBut = cmds.button('clothBut', l='cloth', parent=f, w=35, h=30, bgc=[0.9, 0.9, 0.9],
                               command=lambda *args: self.snakeCommand(clothBut))

        previewBut = cmds.button('previewBut', l='preview', parent=f, w=50, h=30, bgc=[0.9, 0.9, 0.9],
                                 command=lambda *args: self.snakeCommand(previewBut))
        deformBut = cmds.button('deformBut', l='deform', parent=f, w=41, h=30, bgc=[0.9, 0.9, 0.9],
                                command=lambda *args: self.snakeCommand(deformBut))

        solverBut = cmds.button('solvermodelBut', l='solvermodel', parent=f, w=70, h=30, bgc=[0.9, 0.9, 0.9],
                                command=lambda *args: self.snakeCommand(solverBut))
        colliderBut = cmds.button('collider', l='collider', parent=f, w=40, h=30, bgc=[0.9, 0.9, 0.9],
                                  command=lambda *args: self.colliderApp())
        disBut = cmds.button('disBut', l='display/hide', parent=f, w=65, h=30, bgc=[0.9, 0.9, 0.9],
                             command=lambda *args: self.setDisplay())
    ...
    def creatTree(self):
        ...
        for char in treeName:
            if char not in (string.letters + string.digits + '_'):
                cmds.confirmDialog(
                    message=u'命名只能由【英文】、【数字】和【下划】线构成，并且首字母必须是英文。\n请注意项目文件名、贴图和物体名字都只能用这些字符。\n\n发现资产名中输入了不识别的字：" %s "' % char.replace(
                        ' ', u'空格').replace('\t', u'ＴＡＢ'))
                return
        ...
        if typeOMI in [1, 2, 3, 4]:
            ...
            if GRPName != []:
                if cmds.objExists(GRPName[0] + '.treeName') == True:
                    ...
                    if (typeOMI == 1 and treeType == 'c') or (typeOMI == 2 and treeType == 'p') or (
                            typeOMI == 3 and treeType == 's'):
                        ...
                        confirmD = cmds.confirmDialog(
                            message=u'已有组:' + NameStr + u'   是否清除原有组(包括组里物体)创建新组？',
    ...
    def charactersTree(self):
        ...
        proxyGRP = FileType + treeName + '_proxy_group'
        ...
        if cmds.objExists(proxyGRP) == False:
            cmds.group(empty=True, name=proxyGRP, parent=modelGRP)
            self.addTreeAttr(proxyGRP, 'proxy')
            cmds.addAttr(proxyGRP, longName="proxy", at='bool')
            cmds.setAttr("%s.proxy" % proxyGRP, True)
    ...
    def propsTree(self):
        ...
        if cmds.objExists(proxyGRP) == False:
            cmds.group(empty=True, name=proxyGRP, parent=modelGRP)
            cmds.addAttr(proxyGRP, longName='Name', dataType='string')
            cmds.addAttr(proxyGRP, longName='Type', dataType='string')
            cmds.setAttr(proxyGRP + '.Name', 'proxy', type='string', lock=True)
            cmds.setAttr(proxyGRP + '.Type', 'p', type='string', lock=True)
            cmds.addAttr(proxyGRP, longName="proxy", at='bool')
            cmds.setAttr("%s.proxy" % proxyGRP, True)
    ...
    def getGRPName(self):
        ...
        cmds.select(clear=True)
        if cmds.objExists('' + treeName + '_geometry_group') and cmds.getAttr(
                '' + treeName + '_geometry_group.Morh') == 'm':
            ...
            cmds.optionMenu(self.typeOM, edit=True, select=1)
        if cmds.objExists('' + treeName + '_geometry_group') and cmds.getAttr(
                '' + treeName + '_geometry_group.Morh') == 'h':
    ...
    def nameDialog(self):
        result = cmds.promptDialog(title='Rename Object', message='Enter Name:', button=['OK', 'Cancel'],
    ...
    def renameTree_1(self):
        ...
        if cmds.optionMenu(self.typeOM, q=1, sl=1) == 1:
            if cmds.objExists('' + TreeName + '_geometry_group.Morh') and cmds.getAttr(
                    '' + TreeName + '_geometry_group.Morh') == 'm':
                for i in trans:
                ...
                con = cmds.confirmDialog(
                    message=u'大纲中的大组不是人物组，请问要删除当前组(包括组里的物体)并且新建人物组吗?',
    ...
    def renameTree_2(self):
        ...
        if cmds.objExists('' + TreeName + '_geometry_group.Morh') and cmds.getAttr(
            ...
            con = cmds.confirmDialog(message=u'大纲中的大组不是头发组，请问要删除当前组(包括组里的物体)并新建头发组吗?',
                                     button=['OK', 'Cancel'], defaultButton='OK', cancelButton='Cancel')
            return con
    ...
    def fix_attr(self):
        modelName = [x for x in cmds.ls(type='transform') if ('_model_group' in x)]
        if not modelName:
            self.creatTree()



        _geo_name = [x for x in cmds.ls(type='transform') if ('_geometry_group' in x)]
        FileType = ''
        treeName = cmds.textField(self.textFG, query=True, text=True).strip()
        if not treeName:
            cmds.confirmDialog(message=u'请输入名字', button=['Yes'], defaultButton='Yes')
        ...
        if not _geo_name:
            ...
            geometryGRP = _geo_name[0]
            if not cmds.objExists('{}.Name'.format(geometryGRP)):
                # cmds.addAttr(geometryGRP, longName='Name', dataType='string')
                # cmds.addAttr(geometryGRP, longName='Type', dataType='string')
                # cmds.setAttr(geometryGRP + '.Name', 'geometry', type='string', lock=True)
                # cmds.setAttr(geometryGRP + '.Type', 'p', type='string', lock=True)
                # cmds.addAttr(geometryGRP, longName="rendering", at='bool')
                # cmds.setAttr("%s.rendering" % geometryGRP, True)
                self.addTreeAttr(geometryGRP,'geometry')
            if not cmds.objExists('{}.rendering'.format(geometryGRP)):
                cmds.addAttr(geometryGRP, longName="rendering", at='bool')
                cmds.setAttr("%s.rendering" % geometryGRP, True)


        #_preview_grp = [x for x in cmds.ls(type='transform') if ('_preview_group' in x)]
        previewGRP = FileType + treeName + '_preview_group'
        if not cmds.objExists(previewGRP):
        # if cmds.objExists(previewGRP) == False:
            cmds.group(empty=True, name=previewGRP, parent=geometryGRP)
            cmds.addAttr(previewGRP, longName='Name', dataType='string')
            cmds.addAttr(previewGRP, longName='Type', dataType='string')
            ...
            cmds.addAttr(previewGRP, longName="preview", at='bool')
            cmds.setAttr("%s.preview" % previewGRP, True)
        if not cmds.objExists("%s.preview" % previewGRP):
            cmds.addAttr(previewGRP, longName="preview", at='bool')
            cmds.setAttr("%s.preview" % previewGRP, True)


        _solver_name = [x for x in cmds.ls(type='transform') if ('_solver_group' in x)]
        if not _solver_name:
            solverGRP = FileType + treeName + '_solver_group'
            if cmds.objExists(solverGRP) == False:
                cmds.group(empty=True, name=solverGRP, parent=modelGRP)
                ...
                if not cmds.objExists('{}.Name'.format(solverGRP)):
                    self.addTreeAttr(solverGRP, 'solver')
        ...
        if cmds.objExists(cothfieldGRP) == False:
            ...
            if not cmds.objExists('{}.Name'.format(cothfieldGRP)):
                self.addTreeAttr(cothfieldGRP, 'cothfield')
        ...
        if cmds.objExists(solvermodelGRP) == False:
            ...
            if not cmds.objExists('{}.Name'.format(solvermodelGRP)):
                self.addTreeAttr(solvermodelGRP, 'solvermodel')

        deform_name =  [x for x in cmds.ls(type='transform') if ('_deform_group' in x)]
        deformGRP = FileType + treeName + '_deform_group'
        if not deform_name:
            ...
            if not cmds.objExists('{}.Name'.format(deformGRP)):
                self.addTreeAttr(deformGRP, 'deform')

        _proxy = [x for x in cmds.ls(type='transform') if ('_proxy_group' in x)]
        ...
        if not _proxy:
            ...
            if cmds.objExists(proxyGRP) == False:
                cmds.group(empty=True, name=proxyGRP, parent=modelGRP)
                self.addTreeAttr(proxyGRP, 'proxy')
        if cmds.objExists('{}.proxy'.format(proxyGRP)) == False:
            cmds.addAttr(proxyGRP, longName="proxy", at='bool')
            cmds.setAttr("%s.proxy" % proxyGRP, True)
