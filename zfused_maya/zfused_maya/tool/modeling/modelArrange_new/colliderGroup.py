...
class ColliderGroup():
    ...
    def getattr(self):
        window = self.Window
        treeName = ''
        if cmds.objExists('*_model_group'):
    ...
    def creatGrp(self):
        ...
        grpList = ['_nrigid_group', '_ncloth_group', '_nucleus_group', '_collider_group']
        if cmds.objExists( treeName + '_' + groupName + '_group') == False:
            xxxGrp = cmds.group(empty = True, name = treeName + '_' + groupName + '_group')
            cmds.parent(xxxGrp,  treeName + '_' + 'solver_group')
            for i in grpList:
                grpName = cmds.group(empty = True, name = treeName + '_' + groupName + i)
                self.addTreeAttr(grpName,i.split('_')[1])
    ...
    def add(self):
        ...
        targetGroup =treeName + '_' + groupName + '_collider_group'
        newName = treeName + '_' + groupName + '_collider'
        if len(objList) == 0:
            ...
            if cmds.objExists(targetGroup) == True:
                for i in objList:
                    if cmds.listRelatives(i,parent=1) != None:
                        if cmds.listRelatives(i,parent=1)[0] != targetGroup:# python2中的‘<>’不等于运算符，python3中已废弃；‘<>’类似于‘!=’
