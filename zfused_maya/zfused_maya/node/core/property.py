...
def xgen(task_id):
    _project_entity =project_entity(task_id)
...
def get_gpucache():
    ...
    if _gpu_caches:
        for _gpu_cache in _gpu_caches:
            ...
            _production_files = zfused_api.zFused.get("production_file_record", filter = {"Path": _gpu_cache_path})
            if _production_files:
                _production_file = _production_files[-1]
                _task_id = _production_file.get("TaskId")
                _task = zfused_api.task.Task(_task_id)
...
def camera(task_id):
    ...
    _file_code =task_entity.file_code()
