...
def receive_version_file(version_id):
    ...
    if _files:
        ...
        print(_file_name)
        _files = [_file for _file in _files if _file.startswith( _file_name ) and _file.endswith( _suffix )]
...
def assembly_file(task_id, input_tasks = []):
    ...
    if _files:
        ...
        _com_name = "{}".format(_task.file_code())
        _files = [_file for _file in _files if _file.startswith( _com_name ) and _file.endswith( _suffix )]
        if not _files:
            _files = []
        _name = "{}.{:>04d}".format(_name, len(_files))
    else:
    ...
    if _input_tasks:
        if _is_new_attribute_solution:
            ...
            if input_tasks:
                for _input_task in input_tasks:
                    # _argv = {
                    #     "task_id" : _task_id,
                    ...
                    # }
                    _argv = _input_task
                    _argv["task_id"] = _task_id
                    
                    args = (_task_id, _input_task.get("task_attr_input_id"), _input_task.get("input_task_id"), _input_task.get("input_task_attr_output_id"), _input_task.get("namespace"))
                    kwargs = _argv
                    print(args)
                    print(kwargs)

                    _task_attr_input = zfused_api.attr.Input(_input_task.get("task_attr_input_id"))
...
def update_file(task_id, input_tasks = []):
    _task_id = task_id
    if input_tasks:
        for _input_task in input_tasks:
            # _argv = {
