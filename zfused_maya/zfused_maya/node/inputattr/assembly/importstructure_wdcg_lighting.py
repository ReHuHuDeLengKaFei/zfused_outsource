...
if not _is_load:
    try:
        logger.info("try load vray plugin")
    ...
    for parent_node in parent_node_list:
        ...
        if node_type == "assemblyReference":
            ...
            if not _assets:
                ...
                _proxy_file = "{}/shader/vray/maya2018/proxy/{}.mb".format(_production_path, _asset_handle.file_code())
                # _gpu_file = "{}/shader/vray/maya2018/gpu/{}.abc".format(_production_path, _asset_handle.file_code())
                _node_name = "{}_proxy".format(_asset_handle.file_code())

        if node_type == "proxycontainer":
            if name not in global_instance_dict.keys():
                cmds.file(_proxy_file, i = True, ns = ":")
                # _node_name, _, _ = proxycontainer.create_vray_container(_proxy_file, _gpu_file, False)
