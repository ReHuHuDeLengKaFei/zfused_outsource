...
import zfused_maya.node.core.geometrycache as geometrycache
...
def publish_file(*args, **kwargs):
    ...
    try:
        ...
        # publish geometrycache
        _geo_files = geometrycache.files()
        if _geo_files:
            _path_set = geometrycache.paths(_geo_files)[0]
            _intersection_path = max(_path_set)
            geometrycache.publish_file(_geo_files, _intersection_path, _production_path + "/cache/germotry")
            _geo_nodes = geometrycache.nodes()
            if _geo_nodes:
                geometrycache.change_node_path(_geo_nodes, _intersection_path, _production_path + "/cache/germotry")
