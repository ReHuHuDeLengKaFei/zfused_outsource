...
def _create_assembly_node(task_id):
    ...
    _link_handle = zfused_api.objects.Objects(_link_object, _link_id)
    _code = _link_handle.code()   
    ...
    for _assembly_attr in _assembly_attrs:
        ...
        _task = zfused_api.zFused.get("task", filter = {"ProjectEntityType":_link_object,"ProjectEntityId":asset_id,"ProjectStepId":_project_step_id})
        if not _task:
            continue
        _task_id = _task[0]["Id"]
        _task_handle = zfused_api.task.Task(_task_id)
